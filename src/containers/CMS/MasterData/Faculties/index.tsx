...
import { Faculty } from '@services/masterData/faculty/types'
...
export default function Faculties() {
  ...
  const [detailData, setDetailData] = useState<Faculty | null>(null)

  const dataTableHead = [
    { name: 'No' },
    { name: 'Faculty Name' },
    { name: 'Faculty’s Leader' },
    { name: 'Created Date' },
    { name: 'Updated Date' },
    { name: 'Action' },
  ]

  const { data } = useQuery(['faculites', page], async () => service.get(), {
    onError: (error: any) =>
      setSnackbar({
        open: true,
    ...
    <Grid container spacing={3.5}>
      ...
      <Grid item xs={12}>
        <Table dataHead={dataTableHead}>
          <TableBody>
            {data?.content?.map((item: Faculty, index: number) => (
              <TableRow key={index} sx={{ height: 112 }}>
                <TableCell align="center">{index + 1}</TableCell>
                <TableCell align="center">{item.name}</TableCell>
                <TableCell align="center">{item.leader}</TableCell>
